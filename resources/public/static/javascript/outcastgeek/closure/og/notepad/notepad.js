...
goog.provide('og.notepad');
goog.provide('og.notepad.Note');
 ...
 * @return {Array.<og.notepad.Note>} An array containing the resulting
 *     instances.
 */
og.notepad.makeNotes = function(data, noteContainer) {
    var notes = [];
    for (var i = 0; i < data.length; i++) {
        var note =
            new og.notepad.Note(data[i].title, data[i].content, noteContainer);
        notes.push(note);
        note.makeNoteDom();
    }
    return notes;
 ...
 * @constructor
 */
og.notepad.Note = function(title, content, noteContainer) {
    this.title = title;
