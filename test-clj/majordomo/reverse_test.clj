(ns majordomo.reverse-test
  (:use clojure.test
        majordomo.core
        re-rand)
  (:require [majordomo.broker :only [start-broker] :as mdb]
            [majordomo.worker :only [new-worker run as-worker] :as mdw]
            [majordomo.client :only [new-client send!] :as mdc])
